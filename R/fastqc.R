...
fastqc= function(files, mc=TRUE, sampled=FALSE)
{
  # for a single file
  if (length(files)==1)
    res= fastqCpp(files, sampled)
  
  # multi files multi core
  if (length(files) > 1 & mc==TRUE)
  {
    require(parallel)
    res= mclapply(X=files, FUN=fastqCpp, sampled=sampled, mc.preschedule=TRUE, mc.cores=detectCores()+1)  
  }
  
  # multi files single core
  if (length(files) > 1 & mc==FALSE)
  {
    res= lapply(X=files, FUN=fastqCpp, sampled=sampled)
  }
